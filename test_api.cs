...
var alertsResponse = await client.GetAsync("api/v1/alerts/low-stock");
if (!alertsResponse.IsSuccessStatusCode)
{
    Console.WriteLine($"Alerts failed: {alertsResponse.StatusCode}");
    return;
}

var alertsJson = await alertsResponse.Content.ReadAsStringAsync();
Console.WriteLine("API Response:");
Console.WriteLine(alertsJson);

var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
var alerts = JsonSerializer.Deserialize<JsonElement>(alertsJson, options);
